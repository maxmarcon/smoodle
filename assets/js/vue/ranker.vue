...
ul.list-group
	li.d-flex.list-group-item.p-2.align-items-center(v-for="el in elements" :class="{disabled: el.disabled}")
		...
		p-check.p-switch.p-fill(v-if="boolean" v-model="el.value" color="success" :disabled="el.disabled" @change="changed")
		.d-flex(v-else)
			p-radio.p-icon.p-plain(:name="el.name" :value="el.disabled ? null : 1" v-model="el.value" :disabled="el.disabled" toggle @change="changed")
				i.icon.fas.fa-thumbs-up.text-success(slot="extra")
				i.icon.far.fa-thumbs-up(slot="off-extra")
				label(slot="off-label")
			p-radio.p-icon.p-plain(:name="el.name" :value="el.disabled ? null : 0" v-model="el.value" :disabled="el.disabled" toggle @change="changed")
